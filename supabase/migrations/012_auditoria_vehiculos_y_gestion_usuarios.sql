...
-- Extender el trigger existente para cubrir vehiculos
...
-- ─────────────────────────────────────────────────────────────────────
-- Guarda: impide que un superadmin se dé de baja a sí mismo o se quite
-- el rol. Sin esto, el último superadmin activo podría dejar al sistema
-- sin nadie capaz de gestionar usuarios.
...
-- Mantener fecha_baja sincronizada con el campo activo
...
-- Políticas que faltaban.
-- El INSERT lo hace la Edge Function admin-create-user con service role
-- (necesita crear también el usuario en auth.users), así que acá solo
-- habilitamos UPDATE para que el superadmin pueda editar rol y estado.
...
-- El email vive en auth.users, que no es accesible vía PostgREST.
-- Lo desnormalizamos en profiles para poder mostrarlo en la gestión
-- de usuarios sin exponer el schema auth.
...
-- Mejora de handle_new_user: guardar email y leer nombre/rol desde
-- user_metadata cuando el usuario se crea desde la app (Edge Function
-- admin-create-user). Si el metadata no viene (alta manual desde el
-- dashboard de Supabase), cae a los valores por defecto de antes.
